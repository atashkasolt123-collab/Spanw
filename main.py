...
# Каналы и чаты
CHANNEL_USERNAME = "@AdvanceCasino"
CHAT_LINK = "https://t.me/AdvanceChatCasino"
BOT_NAME = "AdvanceCasino"
...
def mines_menu_keyboard():
    buttons = [
        [InlineKeyboardButton(text="▶️ Играть", callback_data="mines_play")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="mini_games"),
         InlineKeyboardButton(text="⚙️ Изменить", callback_data="mines_change")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def game_chats_keyboard():
    buttons = [
        [InlineKeyboardButton(text=f"📢 Наш канал", url=f"https://t.me/{CHANNEL_USERNAME[1:]}")],
        [InlineKeyboardButton(text=f"💬 Наш чат", url=CHAT_LINK)],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_main")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def cmd_start(message: types.Message, command: CommandStart):
    ...
    # В ЧАТАХ ТОЖЕ ИНЛАЙН КНОПКИ, БЕЗ REPLY КЛАВИАТУРЫ!
    await message.answer(
        f"{premium_emoji(PREMIUM_EMOJIS['win'], '👏')} <b>{BOT_NAME}</b>\n\n"
        f"📢 Канал: {CHANNEL_USERNAME}\n"
        f"💬 Чат: {CHAT_LINK}\n\n"
        f"✨ Привет, добро пожаловать в {BOT_NAME}!",
...
async def cmd_menu(message: types.Message):
    await message.answer(
        f"{premium_emoji(PREMIUM_EMOJIS['win'], '👏')} <b>Главное меню {BOT_NAME}</b>",
        reply_markup=main_menu_keyboard()
    )
...
async def cmd_chat(message: types.Message):
    await message.answer(
        f"{premium_emoji(PREMIUM_EMOJIS['game'], '💬')} <b>Наше сообщество {BOT_NAME}</b>\n\n"
        f"📢 Канал: {CHANNEL_USERNAME}\n"
        f"💬 Чат: {CHAT_LINK}\n\n"
        f"✨ Присоединяйся!",
...
async def cmd_top(message: types.Message):
    await message.answer(
        f"{premium_emoji(PREMIUM_EMOJIS['stats'], '📊')} <b>Топ игроков {BOT_NAME}</b>\n\n"
    ...
    try:
        ...
        try:
            await bot.send_message(
                user_id,
                f"{premium_emoji(PREMIUM_EMOJIS['balance'], '💰')} <b>Вам переведено {amount:.2f}$ от администратора {BOT_NAME}</b>"
            )
        except:
            pass
...
async def top_balance_callback(callback: types.CallbackQuery):
    ...
    text = f"{premium_emoji(PREMIUM_EMOJIS['balance'], '💰')} <b>Топ 5 по балансу {BOT_NAME}</b>\n\n"
...
async def top_turnover_callback(callback: types.CallbackQuery):
    ...
    text = f"{premium_emoji(PREMIUM_EMOJIS['stats'], '📊')} <b>Топ 5 по обороту {BOT_NAME}</b>\n\n"
...
async def process_add_balance(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            await bot.send_message(
                user_id,
                f"{premium_emoji(PREMIUM_EMOJIS['balance'], '💰')} Ваш баланс был пополнен на {amount:.2f}$ от администратора {BOT_NAME}"
            )
        except:
            pass
...
async def process_giveaway(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        giveaway_text = (
            f"{premium_emoji(PREMIUM_EMOJIS['prize'], '🎁')} <b>Ежемесячная раздача от {BOT_NAME}!</b>\n\n"
...
async def claim_giveaway(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Ты получил {amount:.2f}$ на баланс от {BOT_NAME}!\n\n"
