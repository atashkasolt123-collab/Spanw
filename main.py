...
• <code>баланс</code> / <code>бал</code> / <code>б</code> - показать баланс
...
async def pay_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if reply_to_message:
        ...
        if target_arg.isdigit():
            ...
            # Это username
            username_search = target_arg[1:]  # Убираем @
            # Ищем пользователя по username (в реальном проекте нужно хранить mapping username->id)
            target_id = None
            target_username = target_arg
            
            await update.message.reply_text(
                "⚠️ Перевод по username временно недоступен.\n"
                "Используйте:\n"
                "1. Ответ на сообщение друга: <code>/pay сумма</code>\n"
                "2. Перевод по ID: <code>/pay ID сумма</code>",
                parse_mode='HTML'
            )
            return
        else:
            await update.message.reply_text("❌ Неверный формат получателя.")
            return
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверяем команды для баланса
    if text in ["баланс", "бал", "б", "balance", "bal", "b"]:
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("баланс", balance_command))
    application.add_handler(CommandHandler("бал", balance_command))
    application.add_handler(CommandHandler("б", balance_command))
    
    application.add_handler(CommandHandler("pay", pay_command))
    application.add_handler(CommandHandler("перевод", pay_command))
    application.add_handler(CommandHandler("перевести", pay_command))
    ...
    print("\n📊 <b>Команды баланса:</b>")
    print("• баланс / бал / б - показать баланс")
