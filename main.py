...
def game_chats_keyboard():
    buttons = [
        [InlineKeyboardButton(text="🇷🇺 Ru - Игровой чат", url="https://t.me/AdvanceChatCasino")],
...
async def cmd_start(message: types.Message, command: CommandStart):
    ...
    if message.chat.type != "private":
        # ДЛЯ ЧАТОВ - ТОЛЬКО КНОПКИ ПОД СООБЩЕНИЕМ
        await message.answer(
            f"{premium_emoji(PREMIUM_EMOJIS['win'], '👏')} Бот работает!"
        )
        
        # Отправляем второе сообщение с обычными кнопками
        ...
        await message.answer(
            f"{premium_emoji(PREMIUM_EMOJIS['win'], '👏')} Привет, добро пожаловать в AdvanceCasino.\n\n"
            f"Подписывайся на наш канал (@AdvanceCasino), чтобы следить за новостями и конкурсами.",
...
# Обработчики для обычных кнопок в чатах
